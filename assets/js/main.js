	...
	function qnt_incre(){
		$(".qtyBtn").on("click", function() {
		  ...
		  var qtyField = $(this).parent(".qtyField"),
			 ...
			 price = $(qtyField).find("#price").val(),
			 id = $(qtyField).find("#pid").val(),
			  newVal = 1;
			console.log(price);
		  if ($(this).is(".plus")) {
				newVal = parseInt(oldValue) + 1;
				tot=parseFloat(tot)+parseFloat(price);
				tot1=parseFloat(tot1)+parseFloat(price);
		  } 
		  else if (oldValue > 1) {
				newVal = parseInt(oldValue) - 1;
				tot=parseFloat(tot)-parseFloat(price);
		  ...
		  document.getElementById('jtot2').innerHTML="$"+tot1;
		  document.getElementById('jtot1').innerHTML="$"+(tot1+10);
		  document.getElementById('tot').value=tot;
		  document.getElementById('tot1').value=tot1;
		  $('#hide').load('cart-handle.php',{
		  	qty:newVal,
		  	id:id
		  });
		});
